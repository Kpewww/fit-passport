...
1. Body-measurement privacy (HIGHEST; the standing governance concern). → Split shareable CLOSET from private BODY PROFILE; body data never exposed by code unless opt-in (default off).
...
Doing the threat model proactively is an ASSET whenever governance is questioned.
